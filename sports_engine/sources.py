...
def _store_raw_payload(config: CompetitionConfig, source_id: str, payload: dict[str, Any]) -> tuple[str, str]:
    ...
    raw = json.dumps(payload, ensure_ascii=False, sort_keys=True).encode("utf-8")
    sha = hashlib.sha256(raw).hexdigest()
    raw_dir = ROOT / "data" / "raw" / config.competition_id / source_id
    raw_dir.mkdir(parents=True, exist_ok=True)
    stamp = utc_now().replace(":", "").replace("+00:00", "Z")
    raw_path = raw_dir / f"{stamp}_{sha[:12]}.json"
    if not raw_path.exists():
        raw_path.write_bytes(raw)
    return raw_path.relative_to(ROOT).as_posix(), sha


def _football_data_rows(payload: dict[str, Any], source_url: str) -> list[dict[str, Any]]:
    """Normalize football-data.org v4 Match resources without guessing team aliases."""
    rows: list[dict[str, Any]] = []
    for match in payload.get("matches", []):
        ...
        rows.append({
            ...
            "source_url": source_url,
            "published_at": match.get("lastUpdated") or utc_now(),
            "collected_at": utc_now(),
        })
    return rows


def fetch_football_data_matches(source: dict[str, Any], config: CompetitionConfig) -> dict[str, Any]:
    """Collect an independent match feed; authentication is supplied only by CI secrets."""
    token_env = str(source.get("token_env", "FOOTBALL_DATA_API_TOKEN"))
    token = os.getenv(token_env, "").strip()
    if not token:
        return {"success": False, "reason": f"environment variable {token_env} is not configured"}
    url = str(source.get("url", "")).strip()
    if not url:
        return {"success": False, "reason": "source URL is empty"}
    payload = _request_json(
    ...
    raw_path, sha = _store_raw_payload(config, str(source.get("id", "football_data_v4")), payload)
    rows = _football_data_rows(payload, url)
    target = config.dataset("external_matches")
    inserted = _append_new_records(
        target,
        rows,
        (("external_match_id",), ("utc_date", "home_team", "away_team")),
    )
    return {
        "success": True,
...
def _weather_value(hourly: dict[str, Any], key: str, index: int) -> Any:
...
def fetch_open_meteo_previous_runs(
    ...
    """Collect forecasts archived before kickoff, never post-match observations.

    The previous-runs fields represent what a model predicted one day before the
    valid hour. This keeps historical replay leakage-safe.
    """
    matches = read_table(config.dataset("matches"))
    venues_path = ROOT / str(source.get("venue_coordinates", "data/context/venue_coordinates.csv"))
    venues = read_table(venues_path, required=False)
    if matches.empty or venues.empty:
        return {"success": False, "reason": "matches or venue coordinates are unavailable"}
    match_id_col = find_column(matches, "jogo", "match_id")
    date_col = find_column(matches, "data", "date")
    time_col = find_column(matches, "horaET", "time_et", "time")
    venue_col = find_column(matches, "estadio", "stadium", "venue")
    venue_name_col = find_column(venues, "estadio", "stadium", "venue")
    lat_col = find_column(venues, "latitude", "lat")
    lon_col = find_column(venues, "longitude", "lon")
    if not all([match_id_col, date_col, time_col, venue_col, venue_name_col, lat_col, lon_col]):
        return {"success": False, "reason": "required match/venue columns are missing"}
    venue_index = {normalize_text(row[venue_name_col]): row for _, row in venues.iterrows()}
    url = str(source.get("url", "https://previous-runs-api.open-meteo.com/v1/forecast"))
    forecast_offset_days = int(source.get("forecast_offset_days", 1))
    variables = [
        f"temperature_2m_previous_day{forecast_offset_days}",
        f"relative_humidity_2m_previous_day{forecast_offset_days}",
        f"precipitation_previous_day{forecast_offset_days}",
        f"wind_speed_10m_previous_day{forecast_offset_days}",
    ]
    requested = set(int(value) for value in match_ids)
    rows: list[dict[str, Any]] = []
    failures: list[dict[str, Any]] = []
    raw_files: list[str] = []
    for _, match in matches.iterrows():
        mid = int(match[match_id_col])
        if mid not in requested:
            continue
        venue = venue_index.get(normalize_text(match[venue_col]))
        if venue is None:
            failures.append({"match_id": mid, "reason": "venue coordinates not mapped"})
            continue
        try:
            local = pd.Timestamp(f"{str(match[date_col])[:10]} {str(match[time_col]).strip()}", tz="America/New_York")
            kickoff = local.tz_convert("UTC")
            params = {
                "latitude": float(venue[lat_col]),
                "longitude": float(venue[lon_col]),
            ...
            raw_path, _ = _store_raw_payload(config, str(source.get("id", "open_meteo_previous_runs")), payload)
            raw_files.append(raw_path)
            hourly = payload.get("hourly") or {}
            times = pd.to_datetime(hourly.get("time", []), utc=True, errors="coerce")
            if not len(times):
                raise ValueError("provider returned no hourly forecast")
            distances = abs(times - kickoff)
            index = int(distances.argmin())
            if distances[index] > pd.Timedelta(hours=2):
                raise ValueError("nearest archived forecast is more than two hours from kickoff")
            suffix = f"previous_day{forecast_offset_days}"
            rows.append({
                "match_id": mid,
                "kickoff_at": kickoff.isoformat(),
                "forecast_valid_at": times[index].isoformat(),
                "forecast_issued_no_later_than": (kickoff - pd.Timedelta(days=forecast_offset_days)).isoformat(),
                "stadium": match[venue_col],
                "latitude": float(venue[lat_col]),
                "longitude": float(venue[lon_col]),
                "temperature_c": _weather_value(hourly, f"temperature_2m_{suffix}", index),
                "relative_humidity_pct": _weather_value(hourly, f"relative_humidity_2m_{suffix}", index),
                "precipitation_mm": _weather_value(hourly, f"precipitation_{suffix}", index),
                "wind_speed_kmh": _weather_value(hourly, f"wind_speed_10m_{suffix}", index),
                "source": f"{url}?{urlencode(params)}",
                "available_at": (kickoff - pd.Timedelta(days=forecast_offset_days)).isoformat(),
                "collected_at": utc_now(),
                "temporal_status": "PRE_MATCH_ARCHIVED_FORECAST",
                "data_quality": "OBSERVED_ARCHIVED_MODEL_RUN",
            })
        except Exception as exc:
            failures.append({"match_id": mid, "reason": f"{type(exc).__name__}: {exc}"})
    target = config.dataset("pre_match_weather")
    inserted = _append_new_records(target, rows, (("match_id",),))
    return {
        "success": bool(rows),
        "matches_received": len(rows),
        "records_inserted": inserted,
        "failures": failures,
