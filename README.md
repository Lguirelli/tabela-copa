...
1. FIFA: ranking oficial e fatos do torneio, importados como snapshots com data de publicação;
2. football-data.org v4: calendário, resultados independentes e histórico estruturado;
3. Hudl StatsBomb Open Data: eventos e escalações históricas para pré-treinamento e validação de features, somente nas competições cobertas;
4. Open-Meteo Previous Runs: previsão meteorológica arquivada que já existia antes do jogo;
5. ESPN: eventos, estatísticas, escalações e dados individuais já integrados.

O sistema nunca converte observação meteorológica pós-jogo em “previsão” histórica e nunca preenche uma informação ausente com valor inventado. Fontes sem cobertura permanecem `NA`. O feed football-data.org é opcional e lê a chave exclusivamente da variável `FOOTBALL_DATA_API_TOKEN`; nenhuma credencial é salva no repositório. A ativação de `enable_online_weight_recalibration` permanece `false` por padrão porque o replay comparativo rejeitou promoção automática sem validação externa.
