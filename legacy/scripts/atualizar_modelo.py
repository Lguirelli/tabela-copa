...
def main():
    ...
    subprocess.run([sys.executable, str(ROOT / "legacy" / "scripts" / "treinar_rede_neural_copa.py")], check=True)
    subprocess.run([sys.executable, str(ROOT / "legacy" / "scripts" / "modelo_neural_diario.py")], check=True)
    subprocess.run([sys.executable, str(ROOT / "legacy" / "scripts" / "recalcular_chaveamento_completo.py")], check=True)
    real_df = read_csv(REAL_CSV)
