...
<<<<<<< HEAD
                ...
                if not weather.empty and "match_id" in weather.columns and "available_at" in weather.columns:
                    eligible_weather = weather[
                        (pd.to_numeric(weather["match_id"], errors="coerce") == match_id)
                        & (pd.to_datetime(weather["available_at"], utc=True, errors="coerce") <= event_time)
                    ]
                    if not eligible_weather.empty:
                        context = eligible_weather.sort_values("available_at").iloc[-1].to_dict()
                pred = model.predict(
